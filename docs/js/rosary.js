...
function rosaryBeadIndex(stepIdx, hailCount) {
    ...
    if (stepIdx === 2) return 1 + hailCount;
    if (stepIdx === 3) return 3;

    const decadeIdx   = Math.floor((stepIdx - 4) / 5);
    const posInDecade = (stepIdx - 4) % 5;
...
function advance() {
    ...
    if (step.type === 'hailMary-group') {
        if (hailMaryCount < step.count - 1) {
            hailMaryCount++;
            renderSlide(step);
            updateBeadTrack();
            return;
    ...
    if (step.type === 'chaplet-repeat') {
        const total = CHAPLET_PRAYERS[step.prayer].repeat;
        if (hailMaryCount < total - 1) {
            hailMaryCount++;
            renderSlide(step);
            updateBeadTrack();
            return;
    ...
    if (step.type === 'chaplet-decade' || step.type === 'sh-decade') {
        if (hailMaryCount < 9) {
            hailMaryCount++;
            renderSlide(step);
            updateBeadTrack();
